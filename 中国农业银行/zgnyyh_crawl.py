...
def process_pc(conn, cursor, session: Session, log_id: int, **kwargs):
    ...
    for type in TYPES_PC:
        ...
        for page in range(1, page + 1, 1):
            ...
            resp = session.request(method=request_method, url=request_url, params=request_params,
                                   headers=HEADERS_PC).text
            # time.sleep(3)
            ...
            if resp and res['status'] == '0000':
                ...
                for product in productList:
                    ...
                    resp = session.request(method=MOBILE_DETAIL_METHOD, url=MOBILE_DETAIL_URL,
                                           data=json.dumps({'productId': productNo}),
                                           headers=HEADERS_MOBILE_DETAIL).text
                    res = json.loads(resp)
                    if resp and res['status'] == '0000':
                        result = res['result']
                        row = {
