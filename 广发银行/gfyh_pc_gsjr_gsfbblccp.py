...
__url = 'http://www.cgbchina.com.cn/Channel/23514821'
__method = 'POST'
...
__field_mapping_funcs = {
    '产品期限（天）': lambda x: str(x) if str(x).endswith('天') else str(x) + '天',
    '业绩比较基准': lambda x: json.dumps({
        'title': '预期收益率',
        'value': x
...
def __to_rows(response):
    ...
    if tables:
        ...
        rows = parse_table(table,
                           ['类型', '产品名称', '产品代码', '产品类型', '产品来源', '风险等级', '合格客户标准', '	起点金额（元）',
                            '币种', '募集起始日', '募集结束日', '产品起息日', '产品到期日', '产品期限（天）', '业绩比较基准',
                            '收益类型', '收费标准', '收费方式'
                            ],
                           callbacks={},
                           extra_attrs={})
