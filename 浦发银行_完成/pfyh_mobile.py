...
from utils.db_utils import init_oracle
from utils.mark_log import insertLogToDB, getLocalDate, updateLogToDB
from utils.global_config import DB_ENV
from requests import Session
from utils.db_utils import update_else_insert_to_db
from utils.spider_flow import SpiderFlow, process_flow
import urllib3

from 浦发银行_完成.pfyh_config import MASK
    ...
    for type in TYPES_MOBILE:
        ...
        if res:
            ...
            for product in productList:
                ...
                resp = session.request(method=MOBILE_DETAIL_METHOD, url=MOBILE_DETAIL_URL,
                                       data=json.dumps(PARAMS_MOBILE_DETAIL), headers=HEADERS_MOBILE_DETAIL).text
                if resp:
                    res = json.loads(resp)
                    result = res['RspBody']
                    row = {
